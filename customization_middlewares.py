...
class HelloMiddleware(CustomMiddleware):
    ...
    async def dispatch(
        ...
        async with context.start(
                action="HelloMiddleware.dispatch",
    ...
    customization=Customization(
        middlewares=[
            ...
            FlowSwitcherMiddleware(
                ...
                oneOf=[CdnSwitch(packageName=name, port=port) for name, port in portsBookFronts.items()],
            ),
            # If no catch from the last middleware, eventually the request's processing flow will be re-routed
            # to a back live-server
            FlowSwitcherMiddleware(
                name="Backend servers",
                oneOf=[RedirectSwitch(origin=f'/api/{name}', destination=f'http://localhost:{port}')
